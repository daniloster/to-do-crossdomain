        ...
        if (!loaded) {
            ...
            app.lazy.directive("lookupMultiple", ['ConfigApp', '$compile', '$parse', function (ConfigApp, $compile, $parse) {
                ...
                templateListSelected = '<div id="listSelected[LOOKUP:ID]" name="listSelected[LOOKUP:ID]" class="chosen-drop anim-slide"><ul class="chosen-results">' +
                    '   <li class="group-option" class="item-selected" ng-repeat="item in selecteds track by item[idProperty] + \'-\' + index">{{ (!!labelProperty ? item[labelProperty] : item.toString()) }}<a href="javascript:;" ng-click="remove(item)"><i class="icon-remove"></i></a></li>' +
                    '</ul></div>';
                return {
                    restrict: 'A',
                    scope: {
